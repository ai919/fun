...
<div style="display: flex; gap: 16px; margin-bottom: 24px;">
    <div style="flex: 1;">
        <div class="admin-card">
            <h2 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600;">广告位列表</h2>
            
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--border-color);">
                        <th style="text-align: left; padding: 8px; font-weight: 600;">标识</th>
                        <th style="text-align: left; padding: 8px; font-weight: 600;">名称</th>
                        <th style="text-align: left; padding: 8px; font-weight: 600;">类型</th>
                        <th style="text-align: left; padding: 8px; font-weight: 600;">显示页面</th>
                        <th style="text-align: center; padding: 8px; font-weight: 600;">状态</th>
                        <th style="text-align: center; padding: 8px; font-weight: 600;">优先级</th>
                        <th style="text-align: center; padding: 8px; font-weight: 600;">操作</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($allAds)): ?>
                        ...
                        <?php foreach ($allAds as $ad): ?>
                            <tr style="border-bottom: 1px solid var(--border-color);">
                                <td style="padding: 8px;">
                                    <code style="font-size: 12px; background: var(--bg-primary); padding: 2px 6px; border-radius: 4px;">
                                        <?= htmlspecialchars($ad['position_key']) ?>
                                    </code>
                                </td>
                                <td style="padding: 8px;"><?= htmlspecialchars($ad['position_name']) ?></td>
                                <td style="padding: 8px;">
                                    <?php
                                ...
                                <td style="text-align: center; padding: 8px;">
                                    <?php if ($ad['is_enabled']): ?>
                                        <span style="color: #059669; font-weight: 600;">✓ 启用</span>
                                    <?php else: ?>
                                ...
                                <td style="text-align: center; padding: 8px;">
                                    <a href="?edit=<?= $ad['id'] ?>" style="color: #2563eb; text-decoration: none; margin-right: 8px;">编辑</a>
                                    <a href="?delete=<?= $ad['id'] ?>" 
                                       onclick="return confirm('确定要删除这个广告位吗？')"
                                       style="color: #dc2626; text-decoration: none;">删除</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <div class="admin-card">
            ...
            <form method="POST" action="">
                ...
                <div style="display: flex; gap: 8px;">
                    <button type="submit" style="flex: 1; padding: 10px; background: #2563eb; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 500;">
                        保存
                    </button>
                    <?php if ($editAd): ?>
                        <a href="ad_positions.php" style="padding: 10px; background: var(--bg-primary); color: var(--text-primary); border: 1px solid var(--border-color); border-radius: 4px; text-decoration: none; text-align: center;">
                            取消
                        </a>
                    <?php endif; ?>
                </div>
